...
- **pbp_play**: One row per play, imported from Pro Football Reference box-score CSVs (1978-present, ~134k rows). Base fields include `pbp_game` (ref to `game`), `pbp_quarter`/`pbp_down`/`pbp_distance`/`pbp_location`, `pbp_detail` (free text), `pbp_player` (single ref to `player`, resolved from free text at import time — often NULL, since resolution is deliberately conservative: only an unambiguous single-player match is recorded), `pbp_scoring_play`/`pbp_scoring_team` (computed from the score delta between consecutive rows), and `pbp_highlight` (single ref to `highlight`, populated by `drush dynasty_plays:match-highlights`, which matches a highlight's game/quarter/down/distance and tie-breaks ambiguous matches using the highlight's tagged players — see `src/Commands/HighlightMatchCommands.php`; re-runnable, never overwrites an existing link). All of `pbp_player`/`pbp_scoring_play`/`pbp_scoring_team`/`pbp_highlight` are deliberately single-value/non-translatable so they stay plain columns on the `pbp_play` base table — see Search Architecture below for why that matters.
