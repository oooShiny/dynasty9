...
class AggregatorController extends ControllerBase {
   ...
   *   The date time service.
   */
  public function __construct(DateFormatterInterface $date_formatter, TimeInterface $date_time) {
  ...
  public function adminOverview() {
    ...
    foreach ($feeds as $feed) {
      ...
      $row[] = ($last_checked ? $this->t('@time ago', ['@time' => $this->dateFormatter->formatInterval($this->dateTime->getRequestTime() - $last_checked)]) : $this->t('never'));
      if (!$last_checked && $refresh_rate) {
        $next_update = $this->t('imminently');
      }
      elseif ($last_checked && $refresh_rate) {
        $next_update = $this->t('%time left', ['%time' => $this->dateFormatter->formatInterval($last_checked + $refresh_rate - $this->dateTime->getRequestTime())]);
      }
      else {
  ...
  public function pageLast() {
    ...
    $build['#attached']['feed'][] = [
      'aggregator/rss',
      $this->config('system.site')->get('name') . ' ' . $this->t('aggregator'),
    ];
